...
/*
Allow packets to be sent wirelessly
 */
//#define RFM12B
...
// R1= 20,000 = - 23.5 gain 
// R1= 24,600 = - 19.1 gain
#define OPAMPGAIN = 470000.0/4700.0

#define VCAL 247.94F
#define ICAL 0.911F
#define PHASECAL 1.0F
...
//In this case 70 samples per full AC wave, if you notice the LCD display going REAL slow reduce this value...
#define NUMBER_OF_SAMPLES_PER_FULLWAVE 70

//Average the power readings over this number of AC samples (25=half second, 50=1 second in UK!)
...
static void takesinglereading() {
  ...
  //Ignore inverted samples - just switch the clamp around
  //This avoids issues where the readings are not perfectly centred at readings of 512
...
typedef struct { 
  int power1, power2, power3, battery; 
} 
...
{  
  ...
  Serial.print(",fltdV ");
    ...
    //show a progress bar on the LCD
    lcd.setCursor ( readingsTaken,0 );
    lcd.print ( char(0));
    lcd.setCursor ( readingsTaken,1 );
    lcd.print ( char(0));
  }  
